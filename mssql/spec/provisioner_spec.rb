...
class VCAP::Services::Mssql::Provisioner
  attr_reader :nodes
end

class VCAP::Services::Mssql::Gateway
  def default_config_file
    File.join(File.dirname(__FILE__), '../config/mssql_gateway_test.yml')
  end
end

describe 'Mssql Provisioner Test' do

  before :each do
    @opts = getProvisionerTestConfig
    EM.run do
      @provisioner = VCAP::Services::Mssql::Provisioner.new(@opts)
      EM.stop
    end
  end
